/* program to receive 1 priority mesages */
#include <stdio.h>
#include <mqueue.h>
#include <time.h>
#include <stdlib.h>
...
mqd_t q;
char *msg;
/* this thread only receive the 1 priority messages */
...
{   
     ...
     mq_receive ( q , buf , MAX_MSG_SIZE ,&msg_prio  );
        if ( msg_prio == 0 ) // if received msg has priority is 0 then send back to qeue 
              mq_send ( q , buf , MAX_MSG_SIZE , 0 );
        else
            printf("%s",buf);
     free(buf);
     }
